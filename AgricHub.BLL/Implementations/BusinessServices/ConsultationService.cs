...
namespace AgricHub.BLL.Implementations.BusinessServices
{
    public class ConsultationService : IConsultationService
    {
        ...
        public async Task<ConsultationResponse> StartConsultationAsync(Guid consultationId)
        {
            ...
            // Clear any pending no-show request when consultant starts
        ...
        public async Task<ConsultationResponse> SubmitCompletionAsync(Guid consultationId, string summary, IFormFile file)
        {
            ...
            // ── CHANGED: accept InProgress and OverdueReview ──────────────────
        ...
        // ── Process expired no-shows (called by background job) ───────────────
        ...
        public async Task ProcessExpiredNoShowsAsync()
        {
            ...
            foreach (var consultation in all)
            {
                ...
                // Grace period expired — process the refund
                var escrow = await FindEscrowAsync(consultation.Id);
                if (escrow != null)
                {
                    var wallet = await _walletRepo.GetSingleByAsync(w => w.CustomerId == consultation.CustomerId);
                ...
                await _sendbirdService.SendNotificationAsync(consultation.Customer.UserId,
                    $"💰 No-show confirmed · ${escrow?.Amount ?? 0} refunded to your wallet · {consultation.Service.ServiceName}",
                    "booking_cancelled");
                await _sendbirdService.SendNotificationAsync(consultation.Consultant.UserId,
                    $"⚠️ No-show processed for {consultation.Service.ServiceName} · No-show count: {consultation.Consultant.NoShowCount}",
                    "noshow_processed");
                await _sendbirdService.SendAdminMessageAsync(consultation.SendbirdChannelUrl,
                    $"❌ No-show grace period expired. ${escrow?.Amount ?? 0} automatically refunded to customer.");
            }

            if (all.Any()) await _unitOfWork.SaveChangesAsync();
        }

        // ── Auto-release expired completion approvals (called by background job) ─
        ...
        public async Task ProcessExpiredApprovalsAsync()
        {
            ...
            foreach (var consultation in all)
            {
                ...
                var consultantWallet = await _walletRepo.GetSingleByAsync(w => w.ConsultantId == consultation.ConsultantId);
                if (consultantWallet == null) continue;
        ...
        // ── Auto-resolve expired customer no-show requests (background job) ─────
        ...
        public async Task ProcessExpiredCustomerNoShowsAsync()
        {
            ...
            foreach (var consultation in all)
            {
                ...
                if (escrow != null)
                {
                    ...
                    if (cWallet != null)
                    {
                        cWallet.Balance    += customerRefund;
                        cWallet.LastUpdated = DateTime.UtcNow;
                        _walletRepo.Update(cWallet);
                    }

                    var conWallet = await _walletRepo.GetSingleByAsync(w => w.ConsultantId == consultation.ConsultantId);
                    if (conWallet != null)
                    {
                        conWallet.Balance    += consultantPayout;
                        conWallet.LastUpdated = DateTime.UtcNow;
                        _walletRepo.Update(conWallet);
                    }
                    ...
                    await _sendbirdService.SendAdminMessageAsync(consultation.SendbirdChannelUrl,
                        $"❌ Customer no-show grace period expired. ${consultantPayout} credited to consultant, ${customerRefund} refunded to customer.");
                    await _sendbirdService.SendNotificationAsync(consultation.Consultant.UserId,
                        $"💰 Customer no-show confirmed · ${consultantPayout} credited to your wallet · {consultation.Service.ServiceName}",
                        "payout_released");
                    await _sendbirdService.SendNotificationAsync(consultation.Customer.UserId,
                        $"⚠️ No-show grace period expired · ${customerRefund} refunded to your wallet · {consultation.Service.ServiceName}",
                        "booking_cancelled");
                }
        ...
        public async Task RescheduleConsultationAsync(Guid consultationId, DateTime newScheduledAt)
        {
            ...
            consultation.NoShowGraceHours       = null;
            consultation.NoShowProcessed        = false;
            // Clear the pending reschedule request — customer has acted on it
            ...
            await _sendbirdService.SendNotificationAsync(consultation.Customer.UserId,
                $"📅 Session rescheduled to {newScheduledAt:MMM d, h:mm tt} · {consultation.Service.ServiceName}",
                "rescheduled");
            await _sendbirdService.SendNotificationAsync(consultation.Consultant.UserId,
                $"📅 Customer rescheduled the session to {newScheduledAt:MMM d, h:mm tt} · {consultation.Service.ServiceName}",
                "rescheduled");
        }
        ...
        public async Task<ConsultationResponse> ReportConsultantNoShowAsync(Guid consultationId)
        {
            ...
            if (escrow != null)
            {
                ...
                w.Balance    += escrow.Amount;
                w.LastUpdated = DateTime.UtcNow;
                _walletRepo.Update(w);
                escrow.Status     = "Refunded";
                escrow.ResolvedAt = DateTime.UtcNow;
                _pendingTransactionRepo.Update(escrow);
                await _walletTransactionRepo.AddAsync(new WalletTransaction
            ...
            await _sendbirdService.SendAdminMessageAsync(consultation.SendbirdChannelUrl,
                $"❌ Consultant no-show confirmed. ${escrow?.Amount ?? 0} refunded.");
            await _sendbirdService.SendNotificationAsync(customer.UserId,
                $"💰 No-show confirmed · ${escrow?.Amount ?? 0} refunded", "booking_cancelled");
        ...
        public async Task<ConsultationResponse> ReportCustomerNoShowAsync(Guid consultationId)
        {
            ...
            if (escrow != null)
            {
                ...
                await _sendbirdService.SendAdminMessageAsync(consultation.SendbirdChannelUrl,
                    $"❌ Customer no-show. ${consultantPayout} credited to consultant, ${customerRefund} refunded to customer.");
                await _sendbirdService.SendNotificationAsync(consultation.Consultant.UserId,
                    $"💰 Customer no-show · ${consultantPayout} credited to your wallet", "payout_released");
            }
        ...
        public async Task RequestCustomerNoShowAsync(Guid consultationId, int graceHours)
        {
            ...
            await _sendbirdService.SendNotificationAsync(
                consultation.Customer.UserId,
                $"⚠️ Consultant reported you as no-show · {consultation.Service.ServiceName} · Respond within {graceHours}h",
                "customer_noshow_request");
        ...
        public async Task DismissCustomerNoShowRequestAsync(Guid consultationId)
        {
            ...
            await _sendbirdService.SendNotificationAsync(
                consultation.Consultant.UserId,
                $"✅ Customer confirmed they are joining · {consultation.Service.ServiceName}",
                "customer_noshow_dismissed");
        ...
        public async Task ProcessExpiredCustomerNoShowForConsultationAsync(Guid consultationId)
        {
            ...
            var expiresAt = consultation.CustomerNoShowRequestedAt!.Value
                .AddHours(consultation.CustomerNoShowGraceHours ?? 5);
            if (DateTime.UtcNow < expiresAt)
        ...
        public async Task RequestRescheduleAsync(Guid consultationId, string reason)
        {
            ...
            await _sendbirdService.SendNotificationAsync(consultation.Customer.UserId,
                $"📅 {consultant.FirstName} {consultant.LastName} requested to reschedule your " +
                $"{consultation.Service.ServiceName} session — please pick a new time",
                "reschedule_request");
        }
        ...
        public async Task<IEnumerable<ConsultationResponse>> GetConsultantConsultationsAsync()
        {
            ...
            // ── Customer stats (Bookings / Completed / Reviews / Avg rating) ─────
            var customerIds = consultations.Select(c => c.CustomerId).Distinct().ToList();

            var allCustomerConsultations = await _consultationRepo.GetByAsync(
                c => customerIds.Contains(c.CustomerId));
            var allCustomerReviews = await _reviewRepo.GetByAsync(
                r => customerIds.Contains(r.CustomerId));

            var bookingsMap = allCustomerConsultations
                .GroupBy(c => c.CustomerId)
                .ToDictionary(g => g.Key, g => g.Count());

            var completedMap = allCustomerConsultations
                .Where(c => c.Status == "Completed")
                .GroupBy(c => c.CustomerId)
                .ToDictionary(g => g.Key, g => g.Count());

            var reviewMap = allCustomerReviews
            ...
            for (int i = 0; i < responses.Count; i++)
            {
                ...
                if (reviewMap.TryGetValue(c.CustomerId, out var rv))
                {
                    r.CustomerReviewCount   = rv.Count;
                    r.CustomerAverageRating = Math.Round(rv.Avg, 1);
                }
                else
                {
                    r.CustomerReviewCount   = 0;
                    r.CustomerAverageRating = 0;
                }
            }
            return responses;
        }

        // ── NEW: Flag overdue InProgress sessions (background sweep) ─────────────
        // After StartedAt + durationMinutes + 2h grace, status → OverdueReview.
        // Both parties are notified. Consultant can still submit from this state.
        ...
        public async Task ProcessOverdueInProgressSessionsAsync()
        {
            ...
            foreach (var consultation in all)
            {
                ...
                var overdueAt = consultation.StartedAt!.Value
                    .AddMinutes(durationMinutes)
                    .AddHours(2); // 2-hour grace before flagging
                ...
                await _sendbirdService.SendNotificationAsync(consultation.Consultant.UserId,
                    $"⏰ Your session for {consultation.Service.ServiceName} is overdue — please submit your work summary",
                    "session_overdue");

                await _sendbirdService.SendNotificationAsync(consultation.Customer.UserId,
                    $"⏰ Your {consultation.Service.ServiceName} session has exceeded its duration. " +
                    $"The consultant has been prompted to submit — you may also report a no-show if they did not attend.",
                    "session_overdue");
            }

            if (all.Any(c => c.Status == "OverdueReview"))
                await _unitOfWork.SaveChangesAsync();
        }
    }
}